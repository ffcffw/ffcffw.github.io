...
<script type="text/javascript"> 
    ...
    //设置延时函数
    function adsUp(){    
        $("#shiping_box").html('<iframe allowFullscreen="true" src="http://v.micool.top/jx/jx.php?url=<?php $result = mysql_query('select * from mkcms_vod where d_id ='.$d_id.'');
    ...
    var t = setTimeout(adsUp,<?php echo $mkcms_miaoshu*1000;?>); 
					...
					<div class="footer clearfix">
						<ul class="cleafix hidden-sm hidden-xs">
									...
									<li><a class="btn btn-sm btn-default" href="<?php echo $mkcms_domain;?>addfav.php?title=<?php echo $d_title ?>&dizhi=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];?>"> <i class="icon iconfont icon-collection"></i> 收 藏</a></li>
...
{	$d_scontent[$i]=explode('|',$d_scontent[$i]);
if($d_scontent[$i][1]!=""){
	echo'<li><a href="http://v.micool.top/jx/jx.php?url='.$d_scontent[$i][1].'" target="ajax" id="'.$d_scontent[$i][0].'">'.$d_scontent[$i][0].'</a></li>';
}
	}
	}else{
		return '';
			...
			<div class="hy-layout clearfix"><div style="margin-top:0px"><?php echo get_ad(13)?></div>
				...
				<div class="tab-content">
					<div class="hy-play-list tab-pane fade in active" id="list3">
						<div class="item">
							<div class="plot">
							<font><?php echo $d_content; ?></font></div>
						</div>
					</div>
				</div>
			</div>
...
<?php echo $mkcms_changyan; ?>
		...
		<script type='text/javascript' src='<?php echo $mkcms_domain;?>style/js/view-history.js'></script>
<script>
			...
			<div class="hy-layout clearfix">
				<div class="hy-details-qrcode side clearfix">
					<div class="item">
						...
						<p>
						<img src="http://qr.liantu.com/api.php?text=<?php echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];?>" width="250">
						</p>
						<p class="text-muted">
							分享到朋友圈
						</p>
				...
				<div class="hy-video-ranking side clearfix">
					<div class="head">
						<a class="text-muted pull-right" href="<?php echo $mkcms_domain;?>vlist.php?cid=0">更多 <i class="icon iconfont icon-xiangyou"></i></i></a>
...
$html="";	
			...
			echo '<li class="text-overflow"><span class="pull-right text-color"><i class="icon iconfont icon-xiangyou"></i></span><a href="'.$ccb.''.$html.'" title="'.$row['d_name'].'">
						<em class="number active ">抢先看资源</em>'.$row['d_name'].'</a></li>';
		}?>			
