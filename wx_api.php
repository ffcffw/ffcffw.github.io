...
{
    //验证签名
    public function valid()//快快影视完美修复公众号可搜索采集的片面
    {
    ...
    //接收文本消息--快快影视完美修复公众号可搜索采集的片面
    private function receiveText($object)
    {
		$a = ($object->Content);
        $keyword = trim($a);//关键字
        $result = mysql_query('select * from mkcms_vod where d_name like "%'.$keyword.'%" order by d_id desc');
		...
		if(count($arr)<=0){
				$arr= "没有你要找的资源，请检查片名是否正确，输入片名时尽量宁少勿错";
}		
