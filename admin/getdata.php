...
';
	if (!empty($title)) {
		foreach ($title as $key => $video) {
			...
			$result = mysql_query('select * from mkcms_vod where d_name= "' . $titile . '"');
			if (mysql_fetch_array($result)) {
				$sql = 'update mkcms_vod set ' . arrtoupdate($_data) . ' where d_name="' . $titile . '"';
				if (mysql_query($sql)) {
				}
				echo '<script>alert("影片重复更新最新地址成功!");window.opener=null;window.close();</script>';
			} else {
				$sql = 'insert into mkcms_vod (' . $str[0] . ') values (' . $str[1] . ')';
				if (mysql_query($sql)) {
...
if (isset($_POST['execute'])) {
	...
	switch ($_POST['execute_method']) {
		case 'srec':
			$sql = 'update mkcms_vod set d_rec = 1 where d_id in (' . $id . ')';
			break;
		case 'crec':
			$sql = 'update mkcms_vod set d_rec = 0 where d_id in (' . $id . ')';
			break;
		case 'shot':
			$sql = 'update mkcms_vod set d_hot = 1 where d_id in (' . $id . ')';
			break;
		case 'chot':
			$sql = 'update mkcms_vod set d_hot = 0 where d_id in (' . $id . ')';
			break;
		case 'delete':
			$sql = 'delete from mkcms_vod where d_id in (' . $id . ')';
			break;
		default:
