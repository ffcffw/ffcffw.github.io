...
if (isset($_POST['save'])) {
	...
	$sql = 'insert into mkcms_player (' . $str[0] . ') values (' . $str[1] . ')';
	if (mysql_query($sql)) {
		$order = mysql_insert_id();
		if ($_POST['order'] == 0) {
			mysql_query('update mkcms_player set order = ' . $order . ' where id = ' . $order);
		}
...
if (isset($_GET['del'])) {
	$sql = 'delete from mkcms_player where id = ' . $_GET['del'] . '';
	if (mysql_query($sql)) {
